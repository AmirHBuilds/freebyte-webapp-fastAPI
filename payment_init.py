...
async def create_invoice(database, user_id, action, amount, cart_id, gateway):
    try:
        ...
        if gateway == 'iran_payment_getway':
            currency = 'IRT'
            gateway_callback_url = settings.computed_iran_callback_url
            payment_nstance = SendInformation(settings.zarinpal_merchent_id)
            create_zarinpal_invoice = await payment_nstance.execute(
                merchent_id=settings.zarinpal_merchent_id, amount=amount, currency=currency, description=action,
            ...
            gateway_callback_url = settings.computed_cryptomus_callback_url
            class_instance = CreateInvoice(settings.cryptomus_api_key, settings.cryptomus_merchant_id)
...
async def verify_cryptomus_payment(order_id: str, uuid_: str | None):
    check_invoic = await InvoiceInfo(settings.cryptomus_api_key, settings.cryptomus_merchant_id).execute(order_id, uuid_)
